...
structure Wires = struct
  ...
  *)
  ...
  fun contains (p : Point.point) : wire -> bool =
    List.exists (fn p' => p = p')
  ...
  fun stepsFromTo
    ...
    (w : wire)
    : int option =
    if not (contains origin w) then NONE
    else if not (contains dest w) then NONE
    else
      let
        fun span' src dst (w' : wire) acc seenSrc =
          case (w', acc, seenSrc) of
               ([], _, _) => []
             | (w''::ws, _, false) => if w''=src
                                      then span' src dst ws [] true
                                      else span' src dst ws [] false
             | (w''::ws, acc', true) => if w''=dst then dst::acc'
                                        else span' src dst ws (w''::acc') true
        fun span src dst w' = span' src dst w' [] false
      in
        SOME (length (span origin dest w))
      end
end
...
fun solution NONE = NONE
  | solution (SOME ws) =
    let
      val stepsToOrigin = Wires.stepsFromTo Point.origin
      val intersections = Wires.intersectionsWithout Point.origin ws
      val distFns = map stepsToOrigin intersections
      val distancesPerIntersection =
        List.mapPartial optionListToListOption
          (map (fn f =>
            map (fn w => f w) ws)
            distFns)
      val sum = foldl (op +) 0
      val distances = map sum distancesPerIntersection
